...
def get_top_n_features(train_X, train_Y, top_n_features):
    ...
    dt_grid = model_selection.GridSearchCV(dt_est, dt_param_grid, n_jobs=25, cv=10, verbose=1,shuffle=True)
    ...
    # 将多个模型选择出的的特征融合
    features_top_n = pd.concat([features_top_n_rf, features_top_n_et, features_top_n_gb, features_top_n_dt], 
                               ignore_index=True).drop_duplicates()
    
    features_importance = pd.concat([feature_imp_sorted_rf, feature_imp_sorted_et, 
