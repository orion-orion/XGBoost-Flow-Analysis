...
def DecodeQuery2(fileName1, fileName2):
    ...
    for item1, item2 in zip(data1, data2):
        ...
        if len(item1) > 50 or len(item1) < 5:
            continue
        item1 = html.unescape(item1)  # 将&gt或者&nbsp这种转义字符转回去
        item1 = parse.unquote(item1)  # 解码,就是把字符串转成gbk编码，然后把\x替换成%。如果
        item1, number = re.subn(r'\d+', "8", item1)  # 正则表达式替换
        item1, number = re.subn(
            r'(http|https)://[a-zA-Z0-9\.@&/#!#\?:]+', "http://u", item1)
...
def readFile():
    ...
    bX1_d = DecodeQuery1(os.path.join(data_save_direct, '网络攻击.csv'))
    bX2_d = DecodeQuery1(os.path.join(data_save_direct, '恶意软件.csv'))
    gX_d = DecodeQuery1(os.path.join(data_save_direct, '业务流量.csv'))
    
    X = vectorizer.fit_transform(bX1_d+bX2_d+gX_d).todense()
    Y = np.array([1]*len(bX1_d)+[2]*len(bX2_d)+[0]*len(gX_d)
                 ).reshape(-1, 1)  # 正常请求标签为0  网络攻击流量标签为1 恶意软件流量标签为2
    ...
    # 读取测试集数据
    X_test_d, time = DecodeQuery2(os.path.join(data_save_direct, '测试流量.csv'), \
        os.path.join(data_save_direct, '时间戳.csv'))
